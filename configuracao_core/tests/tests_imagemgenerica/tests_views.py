...
class TestImagemGenericaViews:
    ...
    def test_imagemgenerica_detail(self, init):
        ...
        url = reverse("configuracao_core:imagemgenerica-detail", args={self.imagemgenerica.pk})
        request = self.factory.get(url)
        request.user = self.user
        response = ImagemGenericaDetailView.as_view()(request, pk=self.imagemgenerica.pk)
        assert response.status_code == 200
    ...
    def test_imagemgenerica_update(self, init):
        ...
        url = reverse("configuracao_core:imagemgenerica-update", args={self.imagemgenerica.pk})
        request = self.factory.put(url)
        request.user = self.user
        response = ImagemGenericaUpdateView.as_view()(request, pk=self.imagemgenerica.pk)
        assert response.status_code == 200

    def test_imagemgenerica_delete(self, init):
        """Teste para a view delete."""
        url = reverse("configuracao_core:imagemgenerica-delete", args={self.imagemgenerica.pk})
        request = self.factory.delete(url)
        setattr(request, "session", "session")
        messages = FallbackStorage(request)
        setattr(request, "_messages", messages)
        request.user = self.user
        response = ImagemGenericaDeleteView.as_view()(request, pk=self.imagemgenerica.pk)
