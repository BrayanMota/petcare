...
class TestImagensSistemaViews:
    ...
    def test_imagenssistema_detail(self, init):
        ...
        url = reverse("configuracao_core:imagenssistema-detail", args={self.imagenssistema.pk})
        request = self.factory.get(url)
        request.user = self.user
        response = ImagensSistemaDetailView.as_view()(request, pk=self.imagenssistema.pk)
        assert response.status_code == 200
    ...
    def test_imagenssistema_update(self, init):
        ...
        url = reverse("configuracao_core:imagenssistema-update", args={self.imagenssistema.pk})
        request = self.factory.put(url)
        request.user = self.user
        response = ImagensSistemaUpdateView.as_view()(request, pk=self.imagenssistema.pk)
        assert response.status_code == 200

    def test_imagenssistema_delete(self, init):
        """Teste para a view delete."""
        url = reverse("configuracao_core:imagenssistema-delete", args={self.imagenssistema.pk})
        request = self.factory.delete(url)
        setattr(request, "session", "session")
        messages = FallbackStorage(request)
        setattr(request, "_messages", messages)
        request.user = self.user
        response = ImagensSistemaDeleteView.as_view()(request, pk=self.imagenssistema.pk)
